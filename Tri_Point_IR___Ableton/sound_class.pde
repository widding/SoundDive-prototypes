class Sound {
  ...
  /*
    ...
    The action is what we're doing to the sound. This can be turn on or off (1), volume (2) or panning (3).
  ...
  */
  ...
  // Constructor
  Sound(String soundName, color soundColor, float soundX, float soundY, float threshold, int soundChannel) {
    name = soundName;
  ...
  void toggleTrack(boolean activated){
    ...
    if (activated){
      ...
      myBus.sendControllerChange(channel, action, 0); // Send a controllerChange
    }
    else{
      //println("Toggled sound on");
      myBus.sendControllerChange(channel, action, 127); // Send a controllerChange
    }
  ...
  void volumeChange(float value){
    ...
    action = 2;
    myBus.sendControllerChange(channel, action, int(value));
  }
  ...
  void getPan(){
    ...
    float panMapped = int(map(pan, 0,180, 127,0));
    // This value should then be sent to Ableton.
    
    /* -----------------
      TO DO
      Send to ableton here...
    ---------------------------- */
